...
2. **The trap/CSR/interrupt column is directed-only, deliberately.** Random
   ...
   the lockstep-vs-Spike simplicity (see GAPS.md on the riscv-dv threshold).
   The cost is that those features get one program's worth of stimulus each.
3. **Misalignment is untested everywhere.** The generator aligns every
   ...
   and the documentation currently makes conflicting claims about it
   (GAPS.md tracks this).
